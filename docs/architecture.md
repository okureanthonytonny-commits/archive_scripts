...
- **`multi_month_zipper.sh`** — run by hand. Loops a range of months,
  calls `single_month_zipper.sh` once per month, stops on real failure,
  skips months whose zip already exists.
...
Both self-relaunch into a detached tmux session with a wake-lock if not
already running inside one — this logic is currently duplicated between
the two files, a known small piece of debt (see `ideas.md`).

## The pipeline: three passes, one hard barrier
...
**Barrier** — `multi_file_pipeline.sh` calls `wait` after pass 1. Nothing
in pass 2 starts until every backgrounded job — fast or slow — has
actually finished. This exists because verifying a video before its
...
```
multi_month_zipper.sh    orchestrator — loop months
...
lib/
  ...
  multi_file_pipeline.sh    process_month() — the 3-pass loop + barrier
